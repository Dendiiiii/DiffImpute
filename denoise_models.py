...
def get_schedule_jump_paper(t_T, jump_length, jump_n_sample):
    jumps = {}
    ...
    class Block(nn.Module):
        def __init__(
            ...
            *,
            ...
            self.mlp = (
                nn.Sequential(
                    nn.SiLU(), nn.Linear(time_emb_dim, d_out * 2, device=device)
                )
    ...
    class Block(nn.Module):
        def __init__(
            ...
            *,
            ...
            self.mlp = (
                nn.Sequential(
                    nn.SiLU(), nn.Linear(time_emb_dim, d_hidden * 2, device=device)
                )
                if exists(time_emb_dim)
                else None
            )
        ...
        def __init__(
            ...
            *,
            ...
            self.linear_second = nn.Linear(
                d_hidden, d_token, bias_second, device=device
            )
            ...
            self.mlp = (
                nn.Sequential(
                    ...
                    nn.Linear(
                        time_emb_dim,
                        d_hidden * (2 if _is_glu_activation(activation) else 1) * 2,
                        device=device,
                    ),
                )
                if exists(time_emb_dim)
    ...
    def __init__(
        ...
        *,
        ...
        def make_kv_compression():
            ...
            return nn.Linear(
                n_tokens,
                int(n_tokens * kv_compression_ratio),
                bias=False,
                device=device,
            )
...
class FTTransformer(nn.Module):
    ...
    def forward(
        self, x_num: Optional[Tensor], x_cat: Optional[Tensor], time=None
    ) -> Tensor:
